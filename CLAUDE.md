...
`install.sh` does not point launchd at `.venv/bin/aw-watcher-ax` directly. Instead, it copies `app_template/aw-watcher-ax.app/` to `~/Applications/aw-watcher-ax.app`, seds the `__BIN_PATH__` placeholder in `Contents/MacOS/aw-watcher-ax` to the venv launcher, ad-hoc codesigns the bundle (`codesign --force --deep --sign - …`), and points the launchd `ProgramArguments` at the binary inside the bundle.

The reason is TCC: macOS's Accessibility permission tracks a code identity. If users grant permission to `python3.11`, the grant breaks the next time Python is upgraded or the venv is rebuilt. By wrapping the launcher in a stable, ad-hoc-signed `.app` bundle and routing launchd through it, TCC tracks the bundle's identity. Users grant permission to `aw-watcher-ax`, the bundle is the responsible code, and the inner exec'd venv binary inherits the grant via the responsibility chain.

Don't change install.sh to launch the venv binary directly — the daemon will start, but on the next venv rebuild Accessibility will silently fall off and the watcher will collect nothing. The wrapper script is intentionally a tiny `exec` shim so the placeholder is easy to sed.
