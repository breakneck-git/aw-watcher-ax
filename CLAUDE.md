...
"For a fixed *toolchain*" is the catch: a new Xcode/CLT version compiles different bytes → different cdhash → the Accessibility grant silently voids on the next reinstall. So `install.sh` rebuilds the trampoline **only when `trampoline.c`'s content hash changed** (stored in `.venv/.trampoline.sha256`); otherwise it stashes and restores the existing binary byte-for-byte, keeping the cdhash stable across reinstalls and toolchain updates. Don't revert this to an unconditional `clang` recompile — that's what made every toolchain bump require a manual re-grant. When the *designated requirement* changes (ad-hoc cdhash changed, or a switch to/from a signing cert), install.sh warns the user to re-grant.

The cdhash binding is only needed for **ad-hoc** signatures. If a stable code-signing identity named `aw-watcher-ax Code Signing` exists in the keychain (created once by `scripts/create-signing-cert.sh`), `install.sh` signs with it instead, and TCC keys the grant on the certificate-based designated requirement (`identifier "com.aw-watcher-ax" and certificate leaf = H"…"`) — which survives *any* rebuild, including a real `trampoline.c` edit. It falls back to ad-hoc when the identity is absent. The cert is self-signed/untrusted (codesign signs anyway; TCC matches the leaf, not a trust chain). When signing with the cert, the trampoline-reuse optimization no longer matters for the grant, but it's kept (cheaper, and helps the ad-hoc fallback path).
